<!-- <div align="center">
<img width="1200" height="475" alt="Kopi Nusantara Banner" src="https://github.com/user-attachments/assets/0aa67016-6eaf-458a-adb2-6e31a0763ed6" />
</div> -->

# Kopi Nusantara – Premium Digital Menu & Brand Site

A blazing‑fast, mobile‑first website for a specialty coffee shop.  
Built as a **portfolio project** to demonstrate modern front‑end craftsmanship, performance optimisation, and real‑world business thinking.

> 🔗 **Live Demo:** [kopinusantara.netlify.app](#)

---

## 🧠 Why this project exists

I created this site to show that I don’t just build pages – I solve business problems.  
Kopi Nusantara needed an online presence that feels as premium as their coffee, loads instantly on any device, and gently guides visitors to visit the store or make a reservation.

This project is part of my freelancing portfolio, focused on the **F&B UMKM niche in Indonesia**.

---

## ✨ Key Features

- **Mobile‑first responsive design** – looks and feels like a native app on phones
- **Digital menu** with smooth category filtering (Minuman, Patisserie)
- **Interactive map & opening hours** (Google Maps embed)
- **Working contact form** with client‑side validation
- **“Order Ahead” button** that opens a WhatsApp chat with a pre‑filled message
- **Performance optimised:** 95+ Lighthouse score on mobile, lazy‑loaded images, critical CSS inlined
- **Lightweight & dependency‑free** – zero frameworks, just vanilla HTML/CSS/JS (unless you used a tool)

---

## 📷 Screenshots

| Homepage Hero | Menu View | Mobile Experience |
|---------------|-----------|-------------------|
| ![Hero](src/assets/Kopi Nusantara Hero.png)    | ![Menu](src/assets/Kopi Nusantara Menu.png)| ![Mobile](src/assets/Kopi Nusantara Mobile.png)      |

*(Add your own screenshots in place of `#` links)*

---

## 🛠 Tech Stack

- **Frontend:** HTML5, CSS3, vanilla JavaScript
- **Hosting:** Netlify
- **Form handling:** [Web3Forms](https://web3forms.com/) (free tier)

---

## 🚀 Run Locally

**Prerequisites:** Node.js installed

1. **Clone the repo**
   ```bash
   git clone https://github.com/actaeon02/coffee-shop-kn-web.git
   cd coffee-shop-kn-web
